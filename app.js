...
// ----------------------------------------------------
// Program Photo & Video Gallery System
// ----------------------------------------------------
const DEFAULT_GALLERY_ITEMS = [
  {
    id: 'g1',
    type: 'photo',
    title: 'Qirat Recitation Stage Performance',
    category: 'Stage Competition',
    src: 'assets/gallery_1.jpg',
    desc: 'Senior student delivering Qirat recitation on the main illuminated Meelad Fest stage.'
  },
  {
    id: 'g2',
    type: 'photo',
    title: 'Arabic Calligraphy Exhibition',
    category: 'Off-Stage Art',
    src: 'assets/gallery_2.jpg',
    desc: 'Detailed close-up of handwritten student Arabic calligraphy created live during competition.'
  },
  {
    id: 'g3',
    type: 'photo',
    title: 'Grand Duffmuttu Performance',
    category: 'Group Event',
    src: 'assets/gallery_3.jpg',
    desc: 'Team Granada performing traditional Duffmuttu with white traditional attire and drums.'
  },
  {
    id: 'g4',
    type: 'video',
    title: 'Meelad Fest Opening Highlights',
    category: 'Video Highlight',
    src: 'assets/gallery_1.jpg',
    duration: '03:45',
    desc: 'Watch video highlights of the inauguration ceremony and opening speeches.'
  },
  {
    id: 'g5',
    type: 'video',
    title: 'Calligraphy & Student Art Showcase',
    category: 'Exhibition Video',
    src: 'assets/gallery_2.jpg',
    duration: '02:30',
    desc: 'Video tour of handwritten magazine preservation and student calligraphy gallery.'
  },
  {
    id: 'g6',
    type: 'video',
    title: 'Duffmuttu & Song Final Performances',
    category: 'Stage Performance',
    src: 'assets/gallery_3.jpg',
    duration: '04:15',
    desc: 'Full stage video highlights of Duffmuttu and group song finals.'
  }
];

let meeladGalleryItems = [...DEFAULT_GALLERY_ITEMS];
const savedGalleryItems = localStorage.getItem('hasaniya_gallery_items');
if (savedGalleryItems) {
  try {
    meeladGalleryItems = JSON.parse(savedGalleryItems);
  } catch (e) {
    console.error("Failed to parse saved gallery items:", e);
  }
}

function saveMeeladGalleryItems() {
  try {
    ...
    console.warn("Storage quota warning when saving gallery items:", e);
  }
}
...
function initGallerySystem() {
  ...
  tabs.forEach(tab => {
    tab.addEventListener('click', () => {
      tabs.forEach(t => t.classList.remove('active'));
      tab.classList.add('active');
      activeGalleryFilter = tab.getAttribute('data-filter');
      renderGalleryGrid();
    });
  });
  ...
  if (closeBtn && modal) {
    closeBtn.addEventListener('click', () => {
      modal.classList.remove('active');
      const body = document.getElementById('galleryModalContent');
  ...
  setupAdminMediaManagement();
  renderGalleryGrid();
}

let pendingMediaFile = null;

function setupAdminMediaManagement() {
  const fileInput = document.getElementById('mediaFileInput');
  const fileBtn = document.getElementById('mediaFileBtn');
  const addForm = document.getElementById('adminAddMediaForm');
  const publishBtn = document.getElementById('adminPublishMediaBtn');
  ...
  if (fileInput) {
    fileInput.addEventListener('change', (e) => {
      ...
      if (file) {
        ...
        // Auto-detect media type based on selected file MIME type
  ...
  function handleMediaPublish(e) {
    ...
    if (!pendingMediaFile && !urlInput) {
      showToast("Please choose a video/photo file or paste a media URL to publish!", "error");
      return;
    }
    ...
    function processAndSave(srcUrl, fileName) {
      if (pendingMediaFile && pendingMediaFile.type.startsWith('video/')) {
        type = 'video';
      }
  ...
  if (addForm) {
    addForm.addEventListener('submit', handleMediaPublish);
  }

  if (publishBtn) {
    publishBtn.addEventListener('click', (e) => {
      handleMediaPublish(e);
    });
  }
...
function renderGalleryGrid() {
  ...
  const filtered = meeladGalleryItems.filter(item => {
    if (activeGalleryFilter === 'photo' && item.type !== 'photo') return false;
  ...
  filtered.forEach(item => {
    const card = document.createElement('div');
    card.className = `gallery-card glass-card ${item.type}-card`;
      ...
      <div class="gallery-img-wrapper">
        ...
        <div class="gallery-overlay">
          <span class="gallery-play-btn">
            <i data-lucide="${isVideo ? 'play' : 'maximize-2'}"></i>
          </span>
          ${item.duration ? `<span class="video-duration-pill"><i data-lucide="clock"></i> ${item.duration}</span>` : ''}
        </div>
        <span class="gallery-type-badge ${item.type}">${item.type.toUpperCase()}</span>
      </div>
  ...
  if (item.type === 'photo') {
    content.innerHTML = `
      <div class="lightbox-img-box">
        <img src="${item.src}" alt="${item.title}" class="lightbox-media">
    ...
    // Video player modal view with playable HTML5 video or YouTube iframe
    const isEmbed = item.src.includes('youtube.com') || item.src.includes('youtu.be');
    ...
    content.innerHTML = `
      <div class="lightbox-video-box">
        ...
        <div class="lightbox-caption">
          <span class="lightbox-cat-badge video"><i data-lucide="video"></i> ${item.category}</span>
          <h3>${item.title}</h3>
          <p>${item.desc}</p>
        </div>
      </div>
